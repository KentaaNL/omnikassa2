...
describe Omnikassa2::OrderAnnounceRequest do
  ...
  context 'when sent' do
    ...
    it 'uses correct URL' do
      order_announce_request.send
      assert_requested :any, 'https://www.example.com/sandbox/order/server/api/order', times: 1
    end

    it 'sets header: \'Content-Type: application/json\'' do
      order_announce_request.send
      assert_requested :any, //, headers: {'Content-Type' => 'application/json'}, times: 1
    end

    it 'sets header: \'Authorization: Bearer <access-token>\'' do
      order_announce_request.send
      assert_requested :any, //, headers: {'Authorization' => 'Bearer myAcCEssT0k3n'}, times: 1
    end
  end
end
